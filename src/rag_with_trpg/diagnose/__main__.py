...
# |   #   | 측정                                 | 왜 다시 재나                                        |✅|❎|
# | :---: | :---------------------------------- | :----------------------------------------------- |
# |   1   | 자/토큰 비율 분포 (평균·최소·하위 10%)     | Week 0 값 1.64~2.12는 문장 7개짜리 표본이다            |✅
# |   2   | 안전 청크 재계산 = `128 × 최소 비율`      | 위가 바뀌면 이 값이 바뀐다                             |✅
# |   3   | 진단 3번 — 초과분 크기 (평균·최대) ← 이월 1 | 초과율만 보면 「1토큰 넘음」과 「3배 넘음」이 같아 보인다         |❎
# |   4   | 페이지별 비율 순위                       | 어느 페이지가 가장 불리한가를 이름으로 안다                |✅
# |   5   | UNK 문자 목록과 빈도 (♻️ 09-03)         | Day 4에서 나온 22종을 2주차 Day 2 축 ②의 입력으로 넘긴다  |✅
# |   6   | 페이지별 헤딩 개수 (♻️ 09-03)           | 3주차 recursive 계단식 설계의 근거. 현재 26/39가 헤딩 1개 |✅
def main() -> None:
    print("[0] diagnose: config environment load")
    load_config()
    config = DiagnoseConfig.from_env()
